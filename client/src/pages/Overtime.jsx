...
function Overtime() {
  ...
  async function fetchOvertime() {
    const r = await axios.get("http://localhost:8000/api/overtime");
    setRecords(r.data);
  ...
  async function saveOvertime() {
    try {
      ...
      await axios.post("http://localhost:8000/api/overtime", { employeeId, hours, rate, date });
  ...
  async function deleteRecord(id) {
    ...
    try { await axios.delete(`http://localhost:8000/api/overtime/${id}`); fetchOvertime(); toast.success("Record deleted"); }
    catch (e) { toast.error("Something went wrong"); }
  }
