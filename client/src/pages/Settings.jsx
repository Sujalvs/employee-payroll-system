...
import { Shield, Users, Download, Trash2, Plus, Clock, RotateCcw, HardDrive, Upload, RefreshCw, AlertTriangle } from "lucide-react";
...
function Settings() {
  ...
  async function resetToDefault() {
    if (!resetPassword) { toast.error("Please enter the reset password"); return; }
  ...
  async function changePassword() {
    ...
    try {
      await axios.post(`${API}/api/auth/change-password`, { username: cpUsername, currentPassword: cpCurrent, newPassword: cpNew });
  ...
  async function saveBackupSettings(updated) {
    try {
      ...
      toast.success(updated.autoBackup ? "Auto backup enabled" : "Auto backup disabled");
  ...
  async function handleRestoreFile(e) {
    ...
    if (!file) return;
    e.target.value = "";
    if (!file.name.endsWith(".db")) { toast.error("Please select a .db backup file"); return; }
    const confirmed = await confirmDialog(
    ...
    try {
      ...
      const r = await axios.post(`${API}/api/backup/restore`, formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  async function exportAllData() {
    try {
      ...
      saveAs(new Blob([buf], { type: "application/octet-stream" }), `Kshethropasana_Backup_${dateStr}.xlsx`);
  ...
  const selectStyle = {
    ...
    padding: "8px 30px 8px 12px", fontSize: "13px", fontFamily: "inherit",
    appearance: "none",
  ...
  return (
    <>
            ...
            <div key={admin.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "var(--bg-hover)", borderRadius: "var(--radius-md)", border: "1px solid var(--border-subtle)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div>
                  ...
                  {admin.id === 1 && <p style={{ fontSize: "11px", color: "var(--text-tertiary)" }}>Primary admin</p>}
                </div>
              </div>
              {admin.id !== 1 && (
      ...
      <div className="form-panel" style={{ marginBottom: "24px" }}>
        ...
        <div style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap", marginBottom: "20px" }}>
          <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
            <div onClick={() => saveBackupSettings({ ...backupSettings, autoBackup: !backupSettings.autoBackup })}
              style={{ width: "44px", height: "24px", borderRadius: "99px", position: "relative", cursor: "pointer", background: backupSettings.autoBackup ? "var(--accent)" : "var(--bg-hover)", border: "1px solid var(--border-default)", transition: "background 0.2s ease" }}>
              <div style={{ position: "absolute", top: "3px", left: backupSettings.autoBackup ? "22px" : "3px", width: "16px", height: "16px", borderRadius: "50%", background: "#fff", transition: "left 0.2s ease", boxShadow: "0 1px 3px rgba(0,0,0,0.3)" }} />
            </div>
            <span style={{ fontSize: "14px", fontWeight: "500" }}>{backupSettings.autoBackup ? "Auto backup is ON" : "Auto backup is OFF"}</span>
          </label>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <span style={{ fontSize: "13px", color: "var(--text-secondary)" }}>Keep backups for</span>
            <select value={backupSettings.keepDays} onChange={e => saveBackupSettings({ ...backupSettings, keepDays: Number(e.target.value) })} style={selectStyle}>
      ...
      <div className="form-panel" style={{ marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button className="add-btn" onClick={createManualBackup} disabled={creatingBackup} style={{ display: "flex", alignItems: "center", gap: "6px", opacity: creatingBackup ? 0.7 : 1 }}>
              <HardDrive size={14} /> {creatingBackup ? "Creating..." : "Back Up Now"}
            </button>
          </div>
        </div>
        {backups.length === 0 ? (
          <div className="empty-state" style={{ padding: "32px" }}>No backups yet — click "Back Up Now" to create the first one</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {backups.map(b => (
              <div key={b.filename} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "var(--bg-hover)", borderRadius: "var(--radius-md)", border: "1px solid var(--border-subtle)", flexWrap: "wrap", gap: "12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
                  <HardDrive size={16} style={{ color: b.filename.includes("auto") ? "var(--accent)" : "var(--green)", flexShrink: 0 }} />
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontWeight: "600", fontSize: "13px", marginBottom: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.filename}</p>
                    <p style={{ fontSize: "12px", color: "var(--text-secondary)" }}>{formatDate(b.createdAt)} · {formatBytes(b.size)}</p>
                  </div>
                </div>
      ...
      <div className="form-panel" style={{ marginBottom: "24px" }}>
        ...
        <input type="file" accept=".db" ref={restoreInputRef} onChange={handleRestoreFile} style={{ display: "none" }} />
        <button className="add-btn" onClick={() => restoreInputRef.current.click()} disabled={restoring}
          ...
          <Upload size={16} /> {restoring ? "Restoring..." : "Upload & Restore .db File"}
        </button>
      </div>
      ...
      <div className="form-panel" style={{ marginBottom: "24px", border: "1px solid rgba(255,69,58,0.25)" }}>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
          <input type="password" placeholder="Enter reset password" value={resetPassword} onChange={e => setResetPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && resetToDefault()}
            style={{ background: "var(--bg-hover)", color: "var(--text-primary)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-md)", padding: "11px 14px", fontSize: "14px", fontFamily: "inherit", width: "260px" }} />
          <button onClick={resetToDefault} disabled={resetting || !resetPassword}
            style={{ background: resetPassword ? "rgba(255,69,58,0.15)" : "var(--bg-hover)", color: resetPassword ? "var(--red)" : "var(--text-tertiary)", border: `1px solid ${resetPassword ? "rgba(255,69,58,0.35)" : "var(--border-subtle)"}`, padding: "11px 22px", borderRadius: "var(--radius-md)", cursor: resetPassword && !resetting ? "pointer" : "not-allowed", fontSize: "14px", fontWeight: "600", fontFamily: "inherit", display: "flex", alignItems: "center", gap: "8px", transition: "all 0.15s ease", opacity: resetting ? 0.7 : 1 }}>
            <RefreshCw size={15} /> {resetting ? "Resetting..." : "Reset All Data"}
          </button>
        </div>
      </div>
      ...
      <div className="form-panel">
        ...
        <p style={{ fontSize: "14px", color: "var(--text-secondary)", marginBottom: "20px" }}>
          Export all data as a human-readable Excel file with separate sheets for each table.
        </p>
