...
function Employees() {
  ...
  async function addOrUpdateEmployee() {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:8000/api/employees/${editingId}`, { name, department, wage, phone, notes });
      } else {
        await axios.post("http://localhost:8000/api/employees", { name, department, wage, phone, notes });
      }
  ...
  async function markInactive(id) {
    ...
    try { await axios.put(`http://localhost:8000/api/employees/inactive/${id}`); fetchEmployees(); toast.success("Employee marked inactive"); }
    catch (e) { console.log(e); }
  }

  async function reactivate(id) {
    try { await axios.put(`http://localhost:8000/api/employees/active/${id}`); fetchEmployees(); toast.success("Employee reactivated"); }
    catch (e) { console.log(e); }
  }

  async function deleteEmployee(id) {
    if (!await confirmDialog("This employee will be permanently deleted.", "Delete")) return;
    try { await axios.delete(`http://localhost:8000/api/employees/${id}`); fetchEmployees(); toast.success("Employee deleted"); if (editingId === id) clearForm(); }
    catch (e) { console.log(e); }
  }
