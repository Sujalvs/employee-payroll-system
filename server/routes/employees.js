  ...
  router.delete("/:id", (req, res) => {
    try {
      const deleteAll = db.transaction((id) => {
        ...
        db.prepare("SELECT * FROM attendance WHERE employeeId=?").all(id).forEach(r => sendToTrash(db, "attendance", `${emp.name} - ${r.date}`, r));
        db.prepare("SELECT * FROM advances WHERE employeeId=?").all(id).forEach(r => sendToTrash(db, "advance", `${emp.name} - Rs.${r.amount}`, r));
        db.prepare("SELECT * FROM overtime WHERE employeeId=?").all(id).forEach(r => sendToTrash(db, "overtime", `${emp.name} - ${r.date}`, r));
        db.prepare("SELECT * FROM payments WHERE employeeId=?").all(id).forEach(r => sendToTrash(db, "payment", `${emp.name} - Rs.${r.amount}`, r));
