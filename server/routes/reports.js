  ...
  router.get("/payroll", (req, res) => {
    ...
    const query = `SELECT e.id, e.name, e.department, e.wage,
      ...
      CASE WHEN ((COALESCE(a.presentDays,0)*e.wage)+COALESCE(ot.totalOvertime,0)-COALESCE(ad.totalAdvance,0)-COALESCE(el.totalEarlyLeave,0))>COALESCE(p.totalPaid,0)
        THEN ((COALESCE(a.presentDays,0)*e.wage)+COALESCE(ot.totalOvertime,0)-COALESCE(ad.totalAdvance,0)-COALESCE(el.totalEarlyLeave,0))-COALESCE(p.totalPaid,0) ELSE 0 END AS remaining,
      CASE WHEN COALESCE(p.totalPaid,0)>((COALESCE(a.presentDays,0)*e.wage)+COALESCE(ot.totalOvertime,0)-COALESCE(ad.totalAdvance,0)-COALESCE(el.totalEarlyLeave,0))
        THEN COALESCE(p.totalPaid,0)-((COALESCE(a.presentDays,0)*e.wage)+COALESCE(ot.totalOvertime,0)-COALESCE(ad.totalAdvance,0)-COALESCE(el.totalEarlyLeave,0)) ELSE 0 END AS excess
      FROM employees e
      ...
      ${project ? "AND e.id IN (SELECT DISTINCT employeeId FROM attendance WHERE project=? AND substr(date,1,4)=? AND substr(date,6,2)=?)" : ""}
    \`;
    const params = project
      ? [yyyy, mm, yyyy, mm, yyyy, mm, yyyy, mm, yyyy, mm, project, yyyy, mm]
