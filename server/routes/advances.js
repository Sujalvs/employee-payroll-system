...
module.exports = function (db) {
  ...
  router.get("/", (req, res) => {
    try {
      res.json(db.prepare(`SELECT a.id, a.employeeId, e.name AS employeeName, a.amount, a.reason, a.date
        FROM advances a JOIN employees e ON a.employeeId=e.id`).all());
  ...
  router.delete("/:id", (req, res) => {
    try {
      ...
      if (item) sendToTrash(db, "advance", `${item.empName} - Rs.${item.amount} - ${item.date}`, item);
